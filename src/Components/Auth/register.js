  ...
  const handleRegister = async () =>{
    ...
    if(data.docs.length > 0){
      ...
      if(!acc.hasOwnProperty('err')){
        console.log(acc); 
        const data1 = await UsersDataService.getUserByWalletAddr(acc.account.networkAccount.addr);
        if(data1.docs.length > 0){
          ...
          appContext.setAppState({...appContext.state,isLoggedin:true,account:acc.account});
